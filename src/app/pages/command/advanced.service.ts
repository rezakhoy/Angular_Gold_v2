 ...
 */
function sort(tables: ICommand[], column: string, direction: string): Command[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<Command[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    constructor(private pipe: DecimalPipe, private service: CommandsService) {
     ...
     * Search Method
     */
    private _search(): Observable<CommandSearchResult> {
        const { sortColumn, sortDirection, pageSize, page, searchTerm } = this._state;

        // 1. sort
        let tables = sort(this.commands, sortColumn, sortDirection);
