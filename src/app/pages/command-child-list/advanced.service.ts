...
import {CommandChild, CommandChildResult, ICommandChild} from "../../core/models/command-child.models";
 ...
 */
function sort(tables: ICommandChild[], column: string, direction: string): CommandChild[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
 ...
 */
function matches(tables: CommandChild, term: string, pipe: PipeTransform) {
    return tables.name.toLowerCase().includes(term.toLowerCase())
        || tables.type.toLowerCase().includes(term.toLowerCase())
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<CommandChild[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    constructor(private pipe: DecimalPipe, private commandService: CommandsService) {
        this.commandService.getCommandChildren().subscribe(res=> {
            this.commandChild = res.body;
            console.log(this.commandChild);
        })
        this._search$.pipe(
     ...
     * Search Method
     */
    private _search(): Observable<CommandChildResult> {
        const { sortColumn, sortDirection, pageSize, page, searchTerm } = this._state;

        // 1. sort
        let tables = sort(this.commandChild, sortColumn, sortDirection);
